...
if selected == 'Home':
    ...
    if st.checkbox('Sharifi & Razaz (2014) Model'):
        ...
        area = st.number_input("Watershed Area (A) in square meters", min_value=0.0, key='area_sharifi')
        dd = st.number_input("Watershed Diameter (DD) in meters", min_value=0.0, key='dd_sharifi')
    ...
    if st.checkbox('Papadakis & Kazan (1986) Model'):
        ...
        length = st.number_input("Watershed Length (L) in meters", min_value=0.0, key='length_papadakis')
        roughness = st.number_input("Surface Roughness (n)", min_value=0.0, key='roughness_papadakis')
        slope = st.number_input("Watershed Slope (S) in meters per meter (m/m)", min_value=0.00,
                                key='slope_papadakis')
        intensity = st.number_input("Rainfall Intensity (i) in mm/h", min_value=0.0,
        ...
        if st.button("Calculate Time of Concentration (Papadakis & Kazan)", key='calculate_papadakis'):
            ...
            st.write(f"**Time of Concentration using Papadakis & Kazan (1986):** {tc_papadakis_kazan:.2f} hours")
